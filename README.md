...
* `-pooling`: The type of pooling layers to use; one of `max` or `avg`. Default is `max`.
  ...
  the option is here.
* `-normalize_weights`: If this flag is present, style and content weights will be divided by the the number of channels for each layer. Idea from [PytorchNeuralStyleTransfer](https://github.com/leongatys/PytorchNeuralStyleTransfer).

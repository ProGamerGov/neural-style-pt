...
#     assert len(extraneous_keys) > 0, 'Following config keys are not recognized: %s' % ', '.join(extraneous_keys)
#     warn(cfg.num_octaves > 1 and cfg.octave_ratio == 1.0, 'Multi-resolution (num_octaves>1) but octave_ratio is 1.0', verbose)
#     assert 'style_image' in cfg or 'style_images' in cfg, 'Error: must specify at least one style image'
#     assert not (stylenet.get_content_weight() > 0 and cfg.content_image is None), 'Error: if no content image provided, content_weight must be '
#     assert cfg.style_blend_weights is None or len(cfg.style_blend_weights) == len(cfg.style_images), 'Error: number of style_blend_weights elements must match number of styles'
#     assert cfg.num_octaves == len(cfg.num_iterations) == len(cfg.style_scale), 'Error: num_octaves does not match length of num_iterations or style_scale lists'

#     # load original content image
#     if 'content_image' not in cfg or cfg.content_image is None:
#         assert isinstance(cfg.size, tuple), 'If no content image provided, config.size must be a full tuple (width, height)'
#         content_image_orig = random_image(cfg.size)
...
#     style_images_aspect = [float(image.shape[3])/image.shape[2] for image in style_images_orig]
    
#     # load original content masks
#     if cfg.content_masks is not None:
#         content_masks_orig = [load_image(mask, cfg.size, to_normalize=False) 
...
#         img = random_image_like(content_image_orig)
#     else:
#         img = input_image #load_image(cfg.input_image, cfg.size)
...
#         image_sizes = [(int(cfg.size * (cfg.octave_ratio ** -s)), 
#                         int(cfg.size * (cfg.octave_ratio ** -s) * aspect)) 
#                        for s in reversed(range(cfg.num_octaves))]
...
#             content_masks = [resize(mask_orig, image_size) 
#                              for mask in cfg.content_masks_orig]
#         else:
#             content_masks = None
...
def style_transfer(stylenet, config, input_image=None, verbose=False):
    ...
    if input_image is None:
        ...
        img = preprocess(input_image) #load_image(cfg.input_image, cfg.size)
